...
// Assign Certificate API
app.post('/assign-certificate', async (req, res) => {
    const { internId, certificateLink } = req.body;
    ...
    try {
        ...
        // Store Certificate
        const certificate = new Certificate({ internId, certificateLink });
        await certificate.save();

        // Send Email
        const mailOptions = {
            from: 'priyanshuraj271204@gmail.com',
            to: intern.email,
            subject: 'Internship Completion Certificate',
            text: `Dear ${intern.name},\n\nCongratulations! You have successfully completed your internship. You can download your certificate from the link below:\n\n${certificateLink}\n\nBest Regards,\nHR Team`
        };

        await transporter.sendMail(mailOptions);
        res.json({ message: "Certificate assigned and email sent!" });
    } catch (error) {
        res.status(500).json({ error: "Error assigning certificate" });
    }
});
